//var crandom = require('./plugins/customrandom')
var random =function(){return Math.random()}//crandom.random
	...
	Generation.prototype.breed = function (g1, g2, nbChilds) {
		...
		for (var nb = 0; nb < nbChilds; nb++) {
			...
			for (var i in data.network.weights) {
				if (random() <= self.options.mutationRate) {
					data.network.weights[i] += random() *
